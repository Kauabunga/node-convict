...
var types = {
  ...
  duration: function duration(x) {
    var err_msg =
      'must be a positive integer or human readable string (e.g. 3000, "5 days")';
...
function _validate(instance, schema, strictValidation) {
  ...
  Object.keys(flatSchema).forEach(function(name) {
    ...
    if (!(name in flatInstance)) {
      try {
        if (
          _typeof(schemaItem.default) === 'object' &&
    ...
    if (
      !(
        typeof schemaItem.default === 'undefined' &&
  ...
  if (strictValidation) {
    Object.keys(flatInstance).forEach(function(name) {
      var err = new Error(
        "configuration param '" + name + "' not declared in the schema"
      );
...
function normalizeSchema(name, node, props, fullName, env, argv, sensitive) {
  ...
  if (o.arg) {
    if (argv[o.arg]) {
      throw new Error(
        "'" + fullName + "' reuses a command-line argument: " + o.arg
      );
    }
    argv[o.arg] = fullName;
  }
  ...
  if (BUILT_INS.indexOf(format) >= 0 || BUILT_IN_NAMES.indexOf(format) >= 0) {
    ...
    if (!types[format]) {
      throw new Error(
        "'" + fullName + "' uses an unknown format type: " + format
      );
    }
    ...
    throw new Error(
      "'" + fullName + "': `format` must be a function or a known format type."
    );
  }
    ...
    validate: function validate(options) {
      ...
      if ('strict' in options) {
        if (options.strict) {
          ...
          deprecate(
            "this syntax is outdated: validate({strict: true}), you must use: validate({allowed: '" +
              ALLOWED_OPTION_STRICT +
              "'})"
          );
        } else {
          deprecate(
            'this syntax is outdated: validate({strict: false}), you must just use: validate()'
          );
        }
      }
